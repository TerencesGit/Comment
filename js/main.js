window.onload = function() {
    ...
    function praise(box, el) {
        ...
        if (txt == '赞') {
            ...
            el.innerHTML = '取消赞';
            praiseEle.innerHTML = (newTotal == 1) ? '我觉得很赞' : '我和' + total + '个人觉得很赞';
        } else {
            newTotal = total - 1;
            praiseEle.setAttribute('total', newTotal)
            el.innerHTML = '赞';
        ...
        praiseEle.style.display = (newTotal == 0) ? 'none' : 'block';
    }
    ...
    function release(box, el) {
        var textarea = box.getElementsByTagName('textarea')[0],
            contentList = box.getElementsByClassName('comment-list')[0],
            contentBox = document.createElement('div');
        contentBox.className = 'comment-box clearfix';
        contentBox.setAttribute('user', 'self');
         html = '<img class="myhead" src="images/my.jpg" alt="" />' +
    ...
    for (var i = 0; i < lis.length; i++) {
        ...
        textarea.onkeyup = function() {
            var len = this.value.length,
                textbox = this.parentNode,
                btn = textbox.children[1],
                word = textbox.children[2];
            if (len == 0 || len > 140) {
                btn.className = 'btn';
